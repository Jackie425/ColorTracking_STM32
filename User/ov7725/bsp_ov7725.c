...
/**
  ...
  * @retval 无
  */
void ImagDisp(uint16_t * Mid_x,uint16_t * Mid_y)
{
	uint16_t i, j;
	int16_t H,S,V;
	uint16_t Camera_Data,Sumcount=0;
	...
	for(i = 0; i < 240; i++)
	{
		for(j = 0; j < 320; j++)
		{
			...
			Img_Rgb565ToHSV(&Camera_Data, &H,  &S,  &V);
			if((H>345||H<15)&&S>30&&V>30)
			{
				x_Count+=j;
				y_Count+=i;
				Sumcount++;
	...
	if(Sumcount>=300)
	{
	*Mid_x=x_Count/Sumcount;
	*Mid_y=y_Count/Sumcount;
	}
	else
	{
		*Mid_x=160;
	...
	printf("\r\nSum=%d Mid_x = %d Mid_y = %d\r\n",Sumcount,*Mid_x,*Mid_y);
}
